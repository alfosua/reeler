...
object InstagramExtractor : Extractor {
    ...
    override suspend fun extract(url: String): MediaInfoExtraction {
        ...
        val media = rawData.data?.xdtShortcodeMedia
            ?: throw ExtractionException("Instagram did not return media info; the post may be private.")
        ...
        val sourceId = postIds.pk.toString()

        val children = media.edgeSidecarToChildren?.edges?.mapNotNull { it.node }
        if (!children.isNullOrEmpty()) {
            // Carousel post: each child is an image or a video of its own.
            val items = children.mapIndexed { index, child ->
                mediaItemOf(child, url, author, caption, "${sourceId}_${index + 1}")
            }
            return MediaInfoExtraction(
                ...
                source = name,
                caption = caption,
                thumbnailUrl = media.thumbnailSrc ?: media.displayUrl,
                author = author,
                items = items,
            )
        }

        return mediaItemOf(media, url, author, caption, sourceId)
    }
    ...
    private fun mediaItemOf(
        ...
        val isVideo = media.isVideo == true && media.videoUrl != null
        val download = if (isVideo) {
            MediaDownloadableExtraction(
    ...
    private suspend fun fetchRawData(post: PostIdentity): RawData {
        ...
        val checkUrl =
            "$API_URL/web/get_ruling_for_content/?content_type=MEDIA&target_id=${post.pk}"
...
private data class GraphqlData(
    ...
    val xdtShortcodeMedia: MediaNode? = null,
)
...
private data class MediaNode(
    ...
    @SerialName("edge_sidecar_to_children")
    val edgeSidecarToChildren: SidecarChildren? = null,
)

@Serializable
private data class SidecarChildren(
    val edges: List<SidecarEdge>? = null,
)

@Serializable
private data class SidecarEdge(
    val node: MediaNode? = null,
)
